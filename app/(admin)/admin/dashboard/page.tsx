...
async function getDashboardData(): Promise<AdminDashboardData> {
  headers(); // <-- MUST be before new Date()
  noStore();
  const now = new Date();
  const from30 = new Date(now);
  from30.setDate(from30.getDate() - 29); // inclusive 30 days window
  ...
  // ---- Time series via SQL (date_trunc) ----
  // Using queryRaw keeps it efficient for large tables.
  ...
  // ---- Distributions (groupBy / queryRaw) ----
  ...
  // Active subscriptions distribution (by subscription name)
  ...
  // Top exams used (by UserExamResult)
  ...
  // ---- Normalize series to include missing days (nice charts) ----
  ...
  const usersSeries30 = allDays30.map((day) => ({
    day,
  ...
  const msgMap = new Map(
    messagesPerDayByRole.map((r) => [msgKey(r.day, r.role), r.count]),
  );
  ...
  return {
    ...
    distributions: {
      ...
      taskPriority: taskPriority.map((x) => ({
        name: x.priority,
        value: x._count._all,
      })),
      feedbackTypes: feedbackTypes.map((x) => ({
        name: x.type,
        value: x._count._all,
      })),
      ...
      activeSubsByName: activeSubsByName.map((x) => ({
        name: x.name,
        value: x.count,
      })),
      ...
      notifications: [
        ...
        {
          name: "Read",
          value: Math.max(0, notificationsTotal - notificationsUnread),
        },
      ],
    },
    recent: {
