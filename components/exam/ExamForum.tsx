...
const ExamForum: React.FC<Props> = ({ exam, token, mode = "grid" }) => {
  ...
  const storageKey = React.useMemo(() => `exam:${exam?.id}:answers`, [exam?.id]);
...
const handleSubmit: React.FormEventHandler<HTMLFormElement> = async (e) => {
  ...
  const userAnswers = Object.entries(answers).map(([questionId, answer]) => ({
    questionId,
    // keep the same serialization your scorer expects:
    // FiveOption/Single: "2"  |  Multi: "[0,2]"  |  Text: "..."  |  Date: "YYYY-MM-DD"
    answer,
  }));
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6">
      ...
      {mode === "grid" ? (
        <div className="grid grid-cols-1 gap-4">
          {questions.map((q, i) => (
            <QuestionCard
              ...
              question={q}
              value={answers[q.id] ?? ""}
              isPreviousOneAnswered={i === 0 ? true : Boolean(answers[questions[i - 1].id])}
        ...
        <div className="flex flex-col gap-4">
          ...
          />
          <div className="flex items-center justify-between">
            <Button type="button" variant="secondary" onClick={goPrev} disabled={idx === 0}>
              قبلی
            </Button>
            <div className="text-sm text-muted-foreground">
              سوال {idx + 1} از {total}
            ...
            {idx < total - 1 ? (
              ...
              <Button type="submit" variant="accent" disabled={loading || !canSubmit}>
                ثبت پاسخ‌ها
              </Button>
            )}
          </div>
